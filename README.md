...
- 주요 메서드
    - boolean add(E e) : 배열의 길이와 상관 없이 요소 하나 추가. E는 자료형
    - int size() : 배열에 추가된 요소 전체 개수를 반환
    - E get(int index) : 배열의 index 위치에 있는 요소 값 반환
    - E remove(int index) : 배열의 index 위치에 있는 요소 값 제거 후 그 값을 반환
    - boolean isEmpty() : 공백 확인
...
- super 예약어 : 하위 클래스에서 상위 클래스로 접근할 때 사용. 상위 클래스의 생성자 호출 가능      
- super. : 상위 클래스의 멤버 변수나 메서드 참고 가능     
### 업캐스팅(클래스 형 변환)
(상위 클래스형) 변수명 = new (하위 클래스형)       
으로 변환이 가능하다. 상위 클래스에만 접근 가능하며 **그 역은 성립되지 않는다.**        
> 메서드 오버라이딩의 경우 인스턴스의 메서드가 호출된다.(하위 클래스형)

## 메서드 오버라이딩

    상위 클래스에서 정의한 메서드를 하위 클래스에서 재정의하는 것
    반환형, 메서드 이름, 매개변수 개수, 매개변수 자료형이 같아야 한다.
    
> Source->Override/Implement Methods로 상속 가능 (Alt+Shift+S)

- 가상 메서드    
    : 변수와 메서드는 사용하는 메모리가 다르므로 변수는 인스턴스가 생성될 때마다 새로 생성되지만, 메서드는 인스턴스가 달라도 같은 로직을 수행한다.
    But, 가상 메서드의 경우 '가상 메서드 테이블'이 만들어지며 각 메서드 이름과 실제 메모리 주소가 짝을 이루고 있음.     
    **자바의 모든 메서드는 가상 메서드**      
